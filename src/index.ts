...
const MODEL_ALIASES =
	`Aliases: "jev-latest" (stable default), "jev-preview" (may move ahead), ` +
	`or a pinned id like "jev-1.13.0" (pin it when confidence thresholds were tuned against a version).`;

const DISCLOSURE =
	"Submitting state and questions sends them to api.typesafe.ai and may incur charges; " +
...
export type Entry = string | Array<unknown> | Record<string, unknown>;
...
export interface JevResult {
	...
	lint: string[];
	estimatedUsd: number;
	/** Score rubrics by question id, carried with the result so rendering is race-free. */
...
function readKeyFromRc(files: string[]): { key: string; source: string } | undefined {
	for (const file of files) {
		try {
			...
			if (last) return { key: last, source: file.replace(/^\/home\/[^/]+|^\/Users\/[^/]+/, "~") };
		} catch {
...
function resolveKey(): { key: string; source: string } | undefined {
	const env = process.env.TYPESAFE_API_KEY?.trim();
	if (env) return { key: env, source: "TYPESAFE_API_KEY (environment)" };
...
function loadToday(): DailyUsage {
	try {
		const parsed = JSON.parse(readFileSync(USAGE_FILE, "utf8")) as DailyUsage;
...
function saveToday(u: DailyUsage): void {
	try {
...
function usdPerInputToken(): number {
...
function isPlainJson(value: unknown, depth = 0, ancestors?: Set<object>): boolean {
...
export function validateEntry(value: unknown, path: string): void {
	if (!isPlainJson(value)) throw new JevError("validation", `${path}: must be plain JSON (string, number, boolean, null, array, or object without cycles).`);
}

export function validateRequest(req: JevRequest): void {
	...
	for (const id of ids) {
		...
		if (!q || typeof q !== "object" || Array.isArray(q)) throw new JevError("validation", `${at}: must be an object with type/instructions/criteria.`);
		if (!["choice", "score", "noul"].includes(q.type)) throw new JevError("validation", `${at}.type: must be "choice", "score", or "noul".`);
		if (q.instructions === undefined || q.instructions === null) throw new JevError("validation", `${at}.instructions: required — put the full judgment in the question text; ids are not sent to the model.`);
		validateEntry(q.instructions, `${at}.instructions`);
		if (q.type === "choice") {
			const c = q.criteria;
			if (!c || typeof c !== "object" || Array.isArray(c)) throw new JevError("validation", `${at}.criteria: choice needs an options map like { billing: "Charges and payments", other: null }.`);
			const keys = Object.keys(c);
			if (keys.length < 1 || keys.length > 64) throw new JevError("validation", `${at}.criteria: 1–64 options required, got ${keys.length}.`);
			if (keys.some((k) => k.length > 200)) throw new JevError("validation", `${at}.criteria: option labels must be at most 200 characters.`);
			for (const k of keys) validateEntry((c as Record<string, unknown>)[k], `${at}.criteria.${k}`);
			...
			for (const [i, level] of c.entries()) validateEntry(level, `${at}.criteria[${i}]`);
		} else if (q.criteria !== undefined && q.criteria !== null) {
			const c = q.criteria as Record<string, unknown>;
			if (typeof c !== "object" || Array.isArray(c)) throw new JevError("validation", `${at}.criteria: noul criteria must be an optional object { true?, false? } describing what yes/no mean.`);
			for (const side of ["true", "false"] as const) {
				if (c[side] !== undefined) validateEntry(c[side], `${at}.criteria.${side}`);
			}
		}
	}
...
export function prepareArguments(args: unknown): unknown {
	if (!args || typeof args !== "object" || Array.isArray(args)) return args;
	const input = args as { state?: unknown; questions?: unknown; model?: unknown };
...
export function lintRequest(req: JevRequest): string[] {
	const warnings: string[] = [];
	for (const [id, q] of Object.entries(req.questions)) {
		if (warnings.length >= MAX_LINT_LINES) break;
		...
		if (q.type === "choice" && q.criteria && typeof q.criteria === "object" && !Array.isArray(q.criteria)) {
			...
			if (keys.length > 16) warnings.push(`choice "${id}" has ${keys.length} options; wide option sets spread probability — consider a two-stage choice.`);
		...
		if (q.type === "score" && Array.isArray(q.criteria)) {
			...
			if (vague) warnings.push(`score "${id}" uses bare degree words (low/medium/高/中等…); describe concrete situations per level instead.`);
		}
		if (warnings.length >= MAX_LINT_LINES) break;
		if (text && text.length < 8) warnings.push(`"${id}" instructions are very short; the question text is the whole program — spell out the full judgment.`);
	}
	if (warnings.length > MAX_LINT_LINES) warnings.length = MAX_LINT_LINES;
	return warnings;
}
...
function validateAnswerShape(req: JevRequest, answers: Record<string, JevAnswer>): void {
	const prob = (v: unknown): v is number => typeof v === "number" && Number.isFinite(v) && v >= 0 && v <= 1;
	for (const [id, q] of Object.entries(req.questions)) {
		const a = answers[id];
		if (!a || a.type !== q.type) throw new JevError("response", `answer for "${id}" is missing or has the wrong type.`);
		if (a.type === "noul" && !prob(a.noul)) throw new JevError("response", `answer "${id}": noul probability out of range.`);
		if (a.type !== "noul" && !prob(a.confidence)) throw new JevError("response", `answer "${id}": confidence out of range.`);
		if (a.probabilities) {
			for (const p of Object.values(a.probabilities)) {
				if (!prob(p)) throw new JevError("response", `answer "${id}": probability out of range.`);
			}
		}
		if (a.type === "choice") {
...
async function abortableSleep(ms: number, signal?: AbortSignal): Promise<void> {
	if (signal?.aborted) throw new JevError("network", "aborted before retry.");
...
async function callOnce(key: string, req: JevRequest, signal?: AbortSignal): Promise<{ answers: Record<string, JevAnswer>; usage: { input_tokens: number; output_tokens: number }; model: string; elapsedMs: number }> {
	const payload = {
		state: req.state,
	...
	try {
		...
		if (signal?.aborted) throw new JevError("network", "aborted.");
	...
	let raw: { model?: unknown; answers?: unknown; usage?: unknown };
	try {
	...
	const usage = raw.usage as { input_tokens?: unknown; output_tokens?: unknown };
	const inputTokens = typeof usage?.input_tokens === "number" ? usage.input_tokens : 0;
	const outputTokens = typeof usage?.output_tokens === "number" ? usage.output_tokens : 0;
	const answers = raw.answers as Record<string, JevAnswer>;
...
export function formatResult(r: JevResult, expanded: boolean): string {
	...
	for (const [id, a] of Object.entries(r.answers)) {
		if (a.type === "noul") {
			...
			const levels = Array.isArray(r.levels?.[id]) ? (r.levels[id] as unknown[]) : [];
	...
	for (const w of r.lint) lines.push(`lint: ${w}`);
...
export default function jevExtension(pi: ExtensionAPI): void {
	...
	const callOut = (ctx: ExtensionContext | undefined, key: string, text: string): void => {
		if (calledOut === key) return;
		calledOut = key;
		try {
			if (ctx?.hasUI) ctx.ui.notify(text, "warning");
			else pi.sendMessage({ customType: "jev-status", content: text, display: true });
		} catch {
			// reporting must never replace the failure it describes
		}
	};

	const report = (ctx: ExtensionContext, text: string, level: "info" | "warning" | "error" = "info"): void => {
	...
	const checkBudget = (): void => {
		...
		if (todayUsage.day !== today()) todayUsage = loadToday();
	...
	const record = (ok: boolean, inputTokens: number): void => {
		todayUsage.requestsStarted += 1;
		if (ok) todayUsage.requestsSucceeded += 1;
		else todayUsage.requestsFailed += 1;
	...
	pi.registerEntryRenderer<JevResult>("jev-result", (entry, { expanded }) =>
		new Text(entry.data ? formatResult({ ...entry.data, lint: entry.data.lint ?? [] }, expanded) : "jev · no result", 0, 0));
	...
	const jevParameters = Type.Object({
		...
		questions: Type.Record(
			...
			{ minProperties: 1, maxProperties: MAX_QUESTIONS },
		),
	...
	pi.registerTool({
		...
		label: "Jev Ask",
		description:
			`Ask TypeSafe's Jev (System One judgment model) structured questions about supplied state; ` +
		...
		prepareArguments: (args) => prepareArguments(args) as Static<typeof jevParameters>,

		async execute(_toolCallId, params, signal, _onUpdate, ctx): Promise<AgentToolResult<JevResult>> {
			if (!enabled) {
			...
			if (!keyInfo) throw new JevError("configuration", "No TypeSafe API key. Set TYPESAFE_API_KEY in the environment or ~/.zshrc.");
			const levels = Object.fromEntries(
				Object.entries(params.questions)
					.filter(([, q]) => q.type === "score" && Array.isArray(q.criteria))
					.map(([id, q]) => [id, q.criteria as unknown[]]),
			);
		...
		renderCall(args) {
			const n = args && typeof args === "object" && args.questions ? Object.keys(args.questions as Record<string, unknown>).length : "?";
		...
		renderResult(result, { expanded, isPartial }) {
			if (isPartial) return new Text("jev_ask · waiting for response", 0, 0);
			if (result.details?.answers) return new Text(formatResult(result.details, expanded), 0, 0);
	...
	const actions = ["enable", "disable", "status", "test", "models"] as const;
	...
	pi.registerCommand("jev", {
		...
		async handler(args, ctx) {
			const action = (args.trim() || "status") as (typeof actions)[number];
			try {
				if (action === "status") {
					const key = resolveKey();
					const caps: string[] = [`session ${sessionRequests}/${sessionMax()} attempts`];
					const perDay = envInt("PI_TYPESAFE_JEV_MAX_REQUESTS_PER_DAY");
					if (perDay !== undefined) caps.push(`${perDay} requests/day`);
					const tokCap = envInt("PI_TYPESAFE_JEV_MAX_INPUT_TOKENS_PER_DAY");
					if (tokCap !== undefined) caps.push(`${tokCap} input tok/day`);
					const usdCap = process.env.PI_TYPESAFE_JEV_MAX_USD_PER_DAY?.trim();
					if (usdCap) caps.push(`$${usdCap}/day`);
					report(ctx,
						`Jev: ${enabled ? "enabled" : "disabled"}. Key: ${key ? key.source : "MISSING"}. ` +
						...
						`Caps: ${caps.join(", ")}. Default model: ${DEFAULT_MODEL}. ${DISCLOSURE}`,
						enabled && !key ? "warning" : "info");
					return;
				}
				if (action === "enable") {
					if (!resolveKey()) { report(ctx, "No TypeSafe API key found. Set TYPESAFE_API_KEY in the environment or ~/.zshrc first.", "warning"); return; }
				...
				if (action === "test") {
					const key = resolveKey();
					if (!key) { report(ctx, "No TypeSafe API key found.", "warning"); return; }
					if (!enabled) enabled = true; // operator-initiated sample implies consent for this one call
					if (!(await ctx.ui.confirm("Send this sample request?", DISCLOSURE))) return;
					const started = Date.now();
					...
					const result: JevResult = {
						...
						levels: { frustration: sample.questions.frustration.criteria as unknown[] },
					};
					record(true, call.usage.input_tokens);
				...
				if (action === "models") {
					...
					if (!response.ok) throw new JevError("http", `HTTP ${response.status} while listing models.`, response.status);
					const body = await response.json() as { data?: Array<{ id?: unknown; name?: unknown; description?: unknown; release_date?: unknown }> };
					const models = Array.isArray(body?.data) ? body.data : [];
					if (models.length === 0) { report(ctx, "Model list is empty (unexpected)."); return; }
